...
var nsGodo_PasswordStrength = function(){
	...
	return {
		// result constant.
		RESULT : {
			length_too_short		: {level : 0, levelText : '사용불가', msg : '비밀번호는 6자 이상 입력해 주세요.'},
		...
		getPasswordStrength : function(pwd){
			...
			N = 0;
			N += symbol.digit ? this.CONFIG.chars.digit.length : 0;
			N += symbol.lower ? this.CONFIG.chars.lower.length : 0;
			N += symbol.upper ? this.CONFIG.chars.upper.length : 0;
			N += symbol.punct ? this.CONFIG.chars.punct.length : 0;
